...
class ProcessorAgent(mesa.Agent):
    ...
    def process_job(self, job, avg_reward, slots = None):
        ...
        costs = self.min_price_per_cpu_second * job['cpu_seconds']

        if is_fulfilled: 
            if slots > 1: 
                self.income += job['reward']/slots
                self.profit += job['reward']/slots - costs
            else: 
    ...
    def get_profit_per_step(self, steps): 
        return self.profit / steps
...
for i in range(10): 
    ...
    for type in model.types: 
        ...
        profits = [p.get_profit_per_step(steps) for p in processors_by_type if p is not None]
        ...
        if type in profits_by_type: 
            profits_by_type[type] += (profits)
        else: 
            profits_by_type[type] = profits
...
for i, type in enumerate(types): 
    ...
    print('MAX profits ' + str(max(profits)))
    print('MIN profits ' + str(min(profits)))
    print('AVG profits ' + str(sum(profits)/len(profits)))
    print('STDev profits ' + str(statistics.stdev(profits)))
    ...
    plt.scatter(profits, reputations, c = colors[i], s=1)
